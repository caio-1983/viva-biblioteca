...
# ============================================================
# Stage 3 — migrate
# Derivado do builder: ambiente completo com Prisma CLI, schema,
# migrations e seeds. Executado como serviço efêmero (restart: "no")
# pelo docker-compose antes da aplicação subir.
# Seeds são idempotentes — seguros em re-deploys.
# ============================================================
FROM builder AS migrate

ENV NODE_ENV=production

CMD ["sh", "-c", "npx prisma migrate deploy && node prisma/seed-init.js && npx tsx prisma/seed-auth.ts"]

# ============================================================
# Stage 4 — runner
# Imagem de produção mínima. Inicia apenas com node server.js.
# Migrações e seeds são executados pelo serviço migrator antes
# do startup (veja docker-compose.yml, serviço migrator).
# ============================================================
